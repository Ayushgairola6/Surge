    ...
    return <>
        <div className="h-screen w-screen flex items-center flex-col sm:flex-row justify-evenly font-mono ">
            ...
            <form  className=" h-full sm:w-1/2 w-full flex items-center  justify-center flex-col gap-2 " action="post">
                ...
                {status==="loading"?<CicularLoader/>:<button onClick={(e)=>handle_Login(e)} className="bg-green-500 text-black w-96 mt-2 p-1 rounded-xl font-bold " >Login</button>
                }
                <span className="mt-4 font-bold text-lg">Don't have an account? <Link className="underline" href="/SignupPage">Sign up</Link></span>
            </form>
            {/* app image */}
            <img className=" h-full w-1/2 hidden sm:block" src="/earth image.jpg" alt="" />
        </div>
